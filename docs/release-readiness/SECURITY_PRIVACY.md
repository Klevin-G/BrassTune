...
- Live observation on 2026-07-12: hosted Render `/api/version` reported `36a225ce0de638771c02bd7d5d7cebb25e6d2871`. The integration work is based on `a2be09eeb9f4cfa4b74e36eef8b07862dd45b091` and is not deployed. Local, PostgreSQL-container, or prior simulator results must not be attributed to the live Render SHA.
...
- A clean Supabase baseline plus the account-deletion/membership-window, invitation-index, usage, pruning, and 2026-07-11 class-code migrations are recorded on the linked project. The three 2026-07-12 membership uniqueness, join-code rotation, and Data API/storage-lockdown migrations passed a local PostgreSQL 17 execution/verification harness on 2026-07-13, but remain unapplied candidate work until exact-SHA CI and a deliberate production migration. FastAPI remains the only application data path.
...
- Native shipping audio defaults are live-microphone-only and deterministic fixtures are test-flag-gated. Live provider/class journeys, physical-device audio, and resolved-revision Xcode tests remain external gates.
