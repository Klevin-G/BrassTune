...
- Pre-release observation on 2026-07-15: hosted Render `/api/version` reported `f19623f0fad932f1b90b952926a93150e7b85970`. Changes in the release containing this update must not be attributed to production until `/api/version` reports that exact pushed release SHA.
- Live Supabase deletion/export was not tested because disposable live credentials were not provided. The schema migrations needed for account deletion durability were applied to project `yznziwewxrlwnwiynlvl`; Render deployment and disposable account/storage/delete journeys remain unverified.
