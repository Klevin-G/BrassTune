...
| [VisionKit document camera](https://developer.apple.com/documentation/visionkit/vndocumentcameraviewcontroller) | Native document scanning should use system document camera where available. | Native Score Practice now presents a VisionKit document-camera flow where supported and a clean unavailable state on unsupported simulator/device contexts. | Physical camera validation on real iPhone/iPad remains. |
