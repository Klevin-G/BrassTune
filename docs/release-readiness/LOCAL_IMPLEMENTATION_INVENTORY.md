...
| `backend/app/tests/test_hardening.py` | Modified | Regression tests for production startup, WebSocket auth/origin/size, payload caps | Expanded auth-mode matrix; `43 passed` targeted, `59 passed` full backend | CI must verify after push | Backend/testing |
