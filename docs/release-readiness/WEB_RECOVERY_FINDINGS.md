...
- Current production hosted smoke still fails because `https://brasstune-u8qj.onrender.com/api/ready` and `/api/version` return 404 from the stale backend.
...
3. Verify `https://brasstune-u8qj.onrender.com/api/version` reports the expected SHA and `https://brasstune-u8qj.onrender.com/api/ready` passes.
...
- Supabase project `yznziwewxrlwnwiynlvl` was `ACTIVE_HEALTHY` on Postgres 17.6.1 before mutation.
