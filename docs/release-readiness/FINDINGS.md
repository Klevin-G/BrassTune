...
| High | Native app was fixture-backed in core product areas. | `swift/BrassTuneApp` now uses a real normal-path `AVAudioEngine` recorder, local playback/delete/export, local Score Practice metadata, and native metronome; deterministic pitch frames remain UI-test-only. | Native repository gate passed locally; App Store/TestFlight, live providers, and physical-device microphone validation remain external. |
