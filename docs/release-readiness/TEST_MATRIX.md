...
| Backend full suite | `cd backend && python -m pytest -q` using a fresh temporary SQLite database | Passed: `160`; skipped: `1` PostgreSQL-only identity regression | Includes multi-class join, self-leave, owner protection, role-reset rejoin, authorization isolation, quotas, abuse limits, races, readiness behavior, and a PostgreSQL check that fixed-ID demo seeding advances but never rewinds generated identities. The PostgreSQL workflow must run all `161` tests. |
