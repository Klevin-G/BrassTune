...
- optional bounded-rotation pair `BRASSTUNE_MAINTENANCE_HMAC_PREVIOUS_KEY_ID` /
  `BRASSTUNE_MAINTENANCE_HMAC_PREVIOUS_KEY`

Never expose `SUPABASE_SECRET_KEY` in Vercel or the browser bundle.
Maintenance retry requests are generated by the private Supabase Cron/Vault
integration documented in `docs/deployment/ACCOUNT_DELETION_MAINTENANCE.md`;
the retired static retry secret must not be configured in production.
