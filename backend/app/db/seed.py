...
def ensure_users_and_group(db: Session) -> None:
    users = [
        User(id=1, name="Avery Brass", role="student", primary_instrument_id="trumpet"),
        User(id=2, name="Jordan Reed", role="director", primary_instrument_id="trombone"),
        User(id=3, name="Maya Chen", role="student", primary_instrument_id="horn"),
        User(id=4, name="Luis Martin", role="student", primary_instrument_id="trombone"),
        User(id=5, name="Sam Rivera", role="student", primary_instrument_id="tuba"),
    ]
    for user in users:
        if db.query(User).filter(User.id == user.id).first() is None:
            db.add(user)
    db.commit()
    ...
    for user_id, instrument_id in members:
        ...
        if exists is None:
            db.add(GroupMember(group_id=1, user_id=user_id, instrument_id=instrument_id))
    db.commit()
...
def _create_seed_session(db: Session, user_id: int, instrument_id: str, days_ago: int, name: str, note_plan: List[Tuple[str, float, float]]) -> None:
    started = dt.datetime.utcnow() - dt.timedelta(days=days_ago, hours=random.randint(0, 5))
